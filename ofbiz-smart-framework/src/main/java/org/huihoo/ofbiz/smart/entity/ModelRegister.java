...
public class ModelRegister implements C{
    ...
    while (iter.hasNext()) {
      ...
      try {
        ...
        for(String className : modelClassNames){
          String table = CommUtil.underscore(className.substring(className.lastIndexOf(".")+1));
  ...
  private Map<String, ColumnMetadata> buildColumnMeta(DatabaseMetaData databaseMetaData,String table) throws SQLException{
